	...
	testCases := []struct {
		...
		},
		{
			...
			test: func(g *WithT, cp *ControlPlane) {
				...
				cp.Machines = NewFilterableMachineCollection(
					machine("1", withValidHash(cp.KCP.Spec), withNeedsRemediationCondition()),
	...
	testCases := []struct {
		...
		},
		{
			...
			test: func(g *WithT, cp *ControlPlane) {
				cp.Machines = NewFilterableMachineCollection(
					machine("1", withNeedsRemediationCondition()),
					machine("2", withNodeRef(), withInfrastructureReady(), withNeedsRemediationCondition()),
				)
...
func TestNextMachineForScaleDown(t *testing.T) {
	...
	m6 := machine("machine-6", withFailureDomain("two"), withTimestamp(startDate.Add(-2*time.Hour)), withValidHash(kcp.Spec), withNeedsRemediationCondition())
